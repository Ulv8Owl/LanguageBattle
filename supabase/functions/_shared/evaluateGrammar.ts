...
/**
 * Включён ли LLM-судья — то есть выставление БАЛЛА моделью.
 *
 * ВЫКЛЮЧЕН ПО УМОЛЧАНИЮ, и это не про «модель не нужна». Модель нужна и
 * работает: она пишет разбор ошибок (_shared/explainElements.ts, флаг
 * EXPLAIN_ENABLED, включён по умолчанию). Выключено именно ОЦЕНИВАНИЕ:
 * балл считает поэлементный разбор (_shared/elementScoring.ts) —
 * детерминированно, бесплатно, за микросекунды и объясняется игроку одной
 * фразой «три части из шести». Оценка модели вместо этого была бы
 * субъективной, платной, медленной и иногда не приходила бы вовсе.
 *
 * Весь код судьи ниже оставлен НЕТРОНУТЫМ и рабочим: это не заглушка, а
 * выключенная ветка. Включается одной переменной окружения Edge Function:
 *
 *     npx supabase secrets set JUDGE_ENABLED=1
 *
 * Понадобиться он может из-за единственного, чего поэлементная оценка не
 * умеет: она не проверяет грамматику и засчитает «I goes to work», если
 * слова совпали.
 */
export const JUDGE_ENABLED = (Deno.env.get("JUDGE_ENABLED") ?? "0") === "1";
