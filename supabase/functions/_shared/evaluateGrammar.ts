...
/**
 * Включён ли LLM-судья.
 *
 * СЕЙЧАС ВЫКЛЮЧЕН ПО УМОЛЧАНИЮ. Оценку считает поэлементный разбор
 * (_shared/elementScoring.ts): он детерминирован, бесплатен, отвечает за
 * микросекунды и объясняется игроку одной фразой — «три части из шести».
 * Судья же субъективен, стоит денег, занимает десятки секунд и иногда не
 * отвечает вовсе.
 *
 * Весь код судьи ниже оставлен НЕТРОНУТЫМ и рабочим: это не заглушка и не
 * мёртвый код, а выключенная ветка. Включается одной переменной окружения
 * Edge Function, без правки кода:
 *
 *     npx supabase secrets set JUDGE_ENABLED=1
 *
 * Когда судья снова понадобится (поэлементная оценка не проверяет
 * грамматику — «I goes to work» она засчитает), достаточно этой команды.
 */
