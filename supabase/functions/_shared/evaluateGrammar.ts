...
/**
 * Судья на тексте: прежний путь, работающий, когда мультимодальная модель
 * выключена (OMNI_ENABLED != 1) или не ответила.
 *
 * Раньше рядом с ним жил флаг JUDGE_ENABLED, потому что балл по умолчанию
 * считал поэлементный разбор, а судью включали отдельно. Поэлементного
 * разбора больше нет, и выключать судью стало нечем в пользу: без него на
 * этом пути не осталось бы никакой оценки вообще. Поэтому флага нет —
 * путь выбирается одним OMNI_ENABLED.
 */
