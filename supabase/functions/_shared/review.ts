...
/**
 * Умеет ли модель «думать» перед ответом — и, значит, принимает ли поле,
 * которым это отключают.
 *
 * ЗАЧЕМ ОТКЛЮЧАТЬ. Режим размышления у этого провайдера гоняет отдельную
 * фазу рассуждения перед ответом: это токены и секунды, а нам нужен один
 * маленький JSON по короткой инструкции. У части моделей семейства Qwen3
 * он вдобавок ВКЛЮЧЁН ПО УМОЛЧАНИЮ, и тогда разбор раунда съедает бюджет
 * задачи на мысли, которых игрок никогда не увидит.
 *
 * ЗАЧЕМ СПИСОК, А НЕ «ОТПРАВИМ ВСЕМ». Модель, которая этого поля не знает,
 * отвечает HTTP 400 и отказом — то есть «на всякий случай» здесь ломает
 * ровно те модели, ради сравнения которых ветка и существует. Поэтому поле
 * уходит только туда, где оно по документации есть: семейство Qwen3 и
 * коммерческие qwen-plus/flash/turbo. Переводчики `qwen-mt-*` и модели
 * других вендоров из списка сюда не входят.
 */
export function supportsThinking(model: string): boolean {
  if (model.startsWith("qwen-mt")) return false;
  if (model.startsWith("qwen3")) return true;
  return ["qwen-plus", "qwen-flash", "qwen-turbo"].includes(model);
}
  ...
  try {
    const res = await fetch(`${judgeBaseUrl()}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        // Размышлять перед ответом не просим НИКОГДА: судье нужен короткий
        // JSON, а не рассуждение. Поле уходит только моделям, которые его
        // знают (см. supportsThinking), и только в текстовых вызовах —
        // мультимодальной оно не адресовано.
        ...(!opts.audio && supportsThinking(model) ? { enable_thinking: false } : {}),
        // Обязателен для аудио-моделей — без него сервис отвечает ошибкой.
        // Текстовым он безразличен, а разбор потока у нас один на всех.
        // Он же обязателен для моделей с размышлением: их провайдер
        // отдаёт только потоком.
        stream: true,
