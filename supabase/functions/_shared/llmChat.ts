 ...
 *
 * ЗАЧЕМ ОТДЕЛЬНЫЙ ФАЙЛ. Здесь собран весь транспорт до текстовой модели:
 * таймаут, откат на повтор, разбор ответа и чтение переменных окружения.
 * Когда этих копий было две (у судьи и у разбора ошибок), смена провайдера
 * означала две одинаковые правки в двух файлах — и разойтись они успели бы
 * на первой же. Разбор ошибок с тех пор переехал к мультимодальной модели,
 * но собирать запрос обратно в вызывающего незачем.
 *
 * ПРОВАЙДЕРЫ:
 *
