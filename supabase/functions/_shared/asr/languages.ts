 ...
 * здесь никто не пытается.
 *
 * СЕЙЧАС НЕ ВЫЗЫВАЕТСЯ. Она была частью проверки «ответил не на том
 * языке», которую сняли вместе с определением языка в ASR: распознаватель
 * теперь слушает ровно тот язык, на который игрок переводит, и вернуть
 * текст чужой письменностью попросту не может — проверка всегда молчала
 * бы. Функция оставлена целой и с тестами, потому что это самый надёжный
 * кирпич для будущей защиты от «прочитал задание вслух вместо перевода»:
 * письменность — факт, а не догадка провайдера.
 *
