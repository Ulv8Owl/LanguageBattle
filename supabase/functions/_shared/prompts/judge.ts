 ...
 *  * Формат ответа. Поля said/fix/kind/why и spoke/correct читает код
 *    (textJudge.ts); переименуете здесь — перестанет читаться.
 *
 * ЧЕМ ОН ОТЛИЧАЕТСЯ ОТ ТОГО ЖЕ ПРОМПТА НА ВЕТКЕ OMNI, И ПОЧЕМУ РОВНО ЭТИМ.
 * Здесь у судьи НЕТ ЗАПИСИ: речь ему уже расшифровал распознаватель, и он
 * читает строку текста. Отсюда три отличия, и других нет:
 *
 *   * полей `audible` и `heard` в ответе нет. Есть ли в записи речь, решает
 *     распознаватель — он её слушал; а расшифровка у нас уже на руках, и
 *     просить модель переписать её значило бы разрешить ей её пригладить.
 *     Лента разбора строится против ТОЙ ЖЕ строки, которую судья прочёл;
 *   * добавлен абзац про артефакты машины: знаки препинания и заглавные
 *     буквы в расшифровке расставил распознаватель, а не игрок, и считать
 *     их ошибками нельзя. На ветке Omni такого абзаца нет и быть не может
 *     — там модель слышит речь сама;
 *   * задание и расшифровка приходят вторым сообщением (textJudge.ts), как
 *     на Omni приходили задание и аудио.
 *
...
in ${v.native} and asked to say it aloud in ${v.target}. You get that sentence and the line a speech
recogniser wrote down from his recording.
${referenceBlock(v)}
...
number words are the same thing said aloud. If a word makes no sense where it stands but sounds close to
the word that belongs there, that is the recogniser mishearing him: leave it alone.

"spoke" — the language that line is written in, in English. If it is not ${v.target}, say so here and
leave "correct" and "errors" empty.

"correct" — HIS OWN SENTENCE WITH ONLY THE WRONG PARTS REPLACED. Start from what he said, not from your own
...
right throughout, "correct" repeats his line and adds only what he left unsaid. It is the WHOLE sentence
either way, complete even if he stopped halfway.
...
Never list what he did NOT say: an omission is already visible from his line, and an entry whose "said"
equals its "fix" is always a mistake on your part. Never mark punctuation, capitalisation or sentence
boundaries: they are the recogniser's, not his. Everything wrong for one reason is one error.
 ...
 * из "correct" — это проверяет код (groundedIn в review.ts), — так что
