...
/**
 * Ссылка на запись для распознавания — открытая по токену и без хвоста.
 *
 * ЗАЧЕМ ОНА ВООБЩЕ. Записи лежат в ЗАКРЫТОМ хранилище: голос это личные
 * данные, и открывать корзину миру нельзя. Провайдер же ходит за файлом
 * сам, без наших заголовков. Поэтому ссылка ведёт на функцию `asr-audio`,
 * которая отдаёт одну запись по подписанному токену на десять минут.
 *
 * ПОЧЕМУ ТОКЕН В ПУТИ, А НЕ В ЗАПРОСЕ. Чтобы ссылка кончалась на `.wav`:
 * так её принимают все проверенные нами модели, тогда как хвост
 * `?token=…` часть из них разбирает неверно. Формат звука при этом едет
 * отдельным полем запроса (см. _shared/asr.ts) — на расширение мы не
 * полагаемся.
 *
 * Не собралась — не беда: остаётся вызов с вложенным аудио.
 */
