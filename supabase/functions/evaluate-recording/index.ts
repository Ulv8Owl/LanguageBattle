...
async function processJob(job_id: string): Promise<void> {
  ...
  try {
    ...
    if (recording.round_id) {
    ...
    } else if (recording.training_round_id && !omni.degraded) {
      ...
      //
      // Когда модель не ответила, БАЛЛ НЕ СТАВИТСЯ. Раньше сюда уходили
      // нейтральные семь, и игрок получал за наш сбой оценку выше
      // половины — по экрану неотличимую от настоящей. Пустой final_score
      // клиент читает как «оцени заново» и возвращает микрофон.
      await supabase
        .from("training_rounds")
