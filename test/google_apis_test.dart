...
void main() {
  ...
  test('все три сервиса ходят через общий резолвер ключа', () {
    // Прямое чтение своей переменной в обход резолвера вернуло бы прежнюю
    // болезнь: один ключ пришлось бы заводить трижды.
    for (final path in [
  ...
  test('LLM по умолчанию — Gemini', () {
    final s = llm();
    expect(s, contains('Deno.env.get("LLM_PROVIDER") ?? "gemini"'));
    expect(s, contains('https://generativelanguage.googleapis.com/v1beta'));
    // Совместимый путь остаётся: смена провайдера — это переменная, а не
    // правка кода.
