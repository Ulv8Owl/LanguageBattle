...
void main() {
  ...
  test('эталон модели не показывают', () {
    ...
    expect(s, contains('There is NO reference answer'));
  ...
  test('модель просят не чинить услышанное', () {
    final s = omni();
    expect(s, contains('including mistakes — do not fix them'));
  ...
  test('ошибки группируются по смыслу, а не по словам', () {
    ...
    expect(omni(), contains('Group errors by MEANING, not by word'));
  });
