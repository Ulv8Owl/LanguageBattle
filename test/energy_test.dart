...
void main() {
  ...
  test('за молчание провайдера и за кэш не платят', () {
    ...
    expect(ts, contains('if (!verdict.degraded) {'));
    // Списание одно, хотя вызова теперь два: игрок платит за разбор, а не
    // за наше устройство пайплайна.
    expect(ts, contains('energy.charge(ENERGY_COST_JUDGE'));
    // Кэша расшифровки нет: хранить её незачем, она нужна одному разбору.
