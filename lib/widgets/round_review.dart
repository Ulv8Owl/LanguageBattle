...
///
/// ЗАЧЕМ ОБЩИЙ. Раньше этот разбор жил только в «Голосе», а в бою
...
/// у соперника вообще другой (в «Общении» родные языки противоположны).
...
class MistakeBreakdown extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // СЕРОЙ ПОДСКАЗКИ НАД ПЛАШКАМИ НЕТ. «Нажми на кусок, чтобы понять, что
    // с ним не так» — это подпись к очевидному: плашки и так выглядят
    // нажимаемыми, у каждой стоит знак вопроса. Строка занимала место над
    // разбором и читалась первой, раньше самих ошибок.
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Wrap(
...
///
/// Ровно то, что показывает «Голос», — и теперь то же самое в бою.
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        // ОШИБОК НЕТ — И НИЧЕГО НЕ ПИШЕМ. Раньше на месте плашек стояла
        // серая строка «Ошибок не найдено — сказано верно» (или про
        // недоговорённую фразу). Она повторяла то, что уже сказано над
