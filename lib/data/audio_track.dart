...
class TimedWord {
  ...
  /// Слово без украшений — таким оно показывается КРУПНО наверху.
  ///
  /// В субтитрах слово приходит вместе со всем, что к нему прилипло:
  /// скобками вокруг звукоподражаний, знаками ноты, запятой в конце строки.
  /// Внизу, в сплошном тексте, это читается нормально — там видно строку
  /// целиком. Наверху слово стоит одно и во весь экран, и «(Мам» или «♪»
  /// там выглядит не словом, а мусором: показывать крупно знак препинания
  /// не за чем.
  ///
  /// ДЕФИС И АПОСТРОФ ВНУТРИ ОСТАЮТСЯ: они часть слова (don't, п-п-п-покер),
  /// а не украшение. Срезаются они только по краям.
  static final RegExp _decoration = RegExp(r"[^\p{L}\p{N}'’-]", unicode: true);
  static final RegExp _edges = RegExp(r"^['’-]+|['’-]+$");

  static String plain(String raw) =>
      raw.replaceAll(_decoration, '').replaceAll(_edges, '');

  /// Само слово, как его стоит показать наверху. Пусто — значит в слове не
  /// было ничего, кроме украшений.
  String get displayText => plain(text);

  /// Перевод там же и по той же причине: скобка под словом выглядит так же
  /// странно, как скобка в слове.
  String get displayTranslation => plain(translation ?? '');
