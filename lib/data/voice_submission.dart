...
enum JudgeStatus {
  ...
  /// Судью не звали: речь не распознана либо это родной слот «Общения».
  skipped,

  /// Игрок говорил не на том языке — модель назвала это сама.
  ///
  /// Значение заведено ещё под проверку языка в распознавании (миграция
  /// 0024). Та проверка ушла вместе с распознавателем, а теперь на тот же
  /// вопрос отвечает сама мультимодальная модель — и это оказалось нужнее:
  /// модель, ожидающая изучаемый язык, в чужой речи «слышит» похожие слова
  /// и записывает их как сказанные.
  wrongLanguage;
...
/// экранах боя и «Голоса» — расхождение между копиями означало бы,
...
///
/// [attemptNumber] — какая это попытка в раунде. Сейчас во всех режимах
/// единственная, но остаётся в имени файла и в строке записи: по нему
/// видно, что запись относится именно к этому раунду.
