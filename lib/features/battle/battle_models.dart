...
class MatchData {
  ...
  ///
  /// В «Общении» сначала ПЕРЕВОД (target), и только потом та же фраза на родном
...
class VoiceRecordingData {
  ...
  /// В бою сейчас не разветвляет вёрстку (проверка «не тот язык» снята),
  /// но остаётся в модели: это единственный признак, по которому видно,
  /// что разбора нет из-за сбоя судьи, а не из-за отсутствия ошибок.
