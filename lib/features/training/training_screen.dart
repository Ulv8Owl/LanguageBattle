...
class _ErrorReport extends StatelessWidget {
  ...
  /// Сказано И верно, И целиком: в ленте нет ни красного, ни зачёркнутого.
  ///
  /// Проверять одно лишь несказанное мало в обе стороны. Игрок, сказавший
  /// одно предложение из двух без единой ошибки, видел «ОШИБОК НЕ НАЙДЕНО»
  /// над красным пропуском и баллом 5. А лишнее слово даёт зачёркнутый кусок
  /// без красного рядом — ответ с ним безупречным тоже не назовёшь.
  bool get _flawless =>
      (attempt?.reviewSpans ?? const []).every((s) => s.kind == 'ok');
  ...
  Widget build(BuildContext context) {
    ...
    final (String title, Color titleColor) = switch (status) {
      ...
      _ when _mistakes.isEmpty && _flawless => ('ОШИБОК НЕ НАЙДЕНО', AppColors.ok),
      ...
      child: Row(
        ...
        child: Column(
          ...
          children: [
            ...
            // плашки с пояснениями от судьи. Вторая попытка показывается
...
/// Кружок с баллом. Пустой — балла ещё нет.
class _ScoreDot extends StatelessWidget {
  final int? score;
  final Color color;

  const _ScoreDot({required this.score, required this.color});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        color: AppColors.navy3,
        border: Border.all(color: score == null ? AppColors.line : color, width: 2.5),
      ),
      child: Center(
        child: Text(
          score == null ? '—' : '$score',
...
class _PipelineDebug extends StatelessWidget {
  ...
  ///
  /// Сколько ошибок судья назвал и сколько мы оставили — расхождение видно
  /// только здесь: часть отсеивают проверки (правка не из перевода, довод
  /// про частотность, цитата не из речи игрока).
  String _scoreText(Map<String, dynamic>? judge) {
    ...
    return [
      ...
      'ошибок ${judge?['errors'] ?? 0}'
          '${judge?['errors_raw'] != null && judge!['errors_raw'] != judge['errors'] ? ' (судья назвал ${judge['errors_raw']})' : ''}',
    ].join('\n');
  }
