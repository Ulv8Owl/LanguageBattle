...
#
# КОНТЕНТ БЕРЁТСЯ ИЗ ТОЙ ЖЕ ВЕТКИ, ЧТО И КОД. Фразы, слова и список записей
# фонотеки лежат не в APK, а в репозитории и тянутся в рантайме
# (RemoteContent), и ветку для них задаёт --dart-define=CONTENT_BRANCH.
# Этого флага здесь не было, то есть любая сборка любой ветки читала контент
# из main: код новый, материал старый. Ломается молча — список записей
# оказывается пустым (формат в main другой, разбор падает в catch), и понять
# по экрану причину нечем.
