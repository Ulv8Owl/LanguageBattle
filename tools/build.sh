...
#
# КОНТЕНТ БЕРЁТСЯ ИЗ ТОЙ ЖЕ ВЕТКИ, ЧТО И КОД. Фразы, слова и список треков
# лежат не в APK, а в репозитории и тянутся в рантайме (RemoteContent), и
# ветку для них задаёт --dart-define=CONTENT_BRANCH. Этого флага здесь не
# было, то есть любая сборка любой ветки читала контент из main: код новый,
# материал старый. Спасал только откат на бандл — то есть ровно до первого
# файла, который в main всё-таки есть. Ломалось бы молча: список треков
# оказался бы пустым, и понять по экрану причину нечем.
...
step "5/9 Проверяю контент и код перед сборкой"
# Глоссарий Тренировки собирается ИЗ ИСХОДНИКА при каждой сборке, а не
# правится в собранном JSON: расходятся они молча, и заметно это стало бы
# только на живой карточке с переводом не от того слова.
python3 tools/build_glossary.py \
  || fail "глоссарий не собрался — смотри список выше."
...
flutter build apk --release \
  ...
  --dart-define=CONTENT_BRANCH="$BRANCH" \
