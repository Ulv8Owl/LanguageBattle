"""Общие функции для tools/build_content.py.

Формат исходников содержимого (content/<язык>/words_<уровень>.txt): одно
слово на строку, в порядке, СОВПАДАЮЩЕМ С АНГЛИЙСКИМ ФАЙЛОМ ТОГО ЖЕ
УРОВНЯ — строка N любого языка это перевод строки N английского. Порядок и
есть ключ соответствия, отдельного id нет: добавить слово значит дописать
строку в конец файла на ВСЕХ языках уровня сразу, а не только на одном.

ФРАЗЫ СЮДА БОЛЬШЕ НЕ ОТНОСЯТСЯ: они живут в assets/cefr (с разбиением на
элементы и пояснениями) и собираются tools/build_cefr.py.
...
def content_file(language: str, kind: str, level: str) -> pathlib.Path:
    """kind — сейчас всегда 'words'; параметр оставлен, потому что
    имя файла собирается из него же."""
