...
branch_note() {
  case "$1" in
    ...
    LLM)      echo "распознавание речи + текстовый судья" ;;
    Exp2)     echo "Аудирование: локальный трек и подтягивание субтитров по ссылке" ;;
    *)        echo "" ;;
  esac
}
...
# молчит о той, ради которой скрипт и запускают.
KNOWN_BRANCHES=(main features LLM Exp2)
